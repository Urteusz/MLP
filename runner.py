...
def run_classification(
        name="Iris",
        architecture=None,  # Ważne dla autoenkodera, np. [4, 2, 4]
        use_bias=True,
        learning_rate=0.1,
        momentum=0.2,
        max_epochs=None,
        ...
        log_interval=20,
        trained_model_file="iris_mlp_model.pkl"  # Domyślne dla klasyfikatora Iris
):
    training_log_file = (
    ...
    if name == "Iris":
        ...
        # Dane dla autoenkodera (np. 4 wejścia, chcemy uzyskać np. 2-wymiarową reprezentację)
        # Architektura powinna być odpowiednio dostosowana, np. [4, 2, 4]
        train_data = [
        ...
        test_data = train_data  # Dla tego autoenkodera, test jest taki sam jak trening
        IRIS_CLASS_NAMES = None  # Nie dotyczy autoenkodera
        if architecture is None:  # Domyślna architektura dla tych danych
            architecture = [4, 2, 4]
    ...
    avg_final_error_train = None  # Przechowamy finalny błąd treningowy
    ...
    if name == "Autoencoder":
        ...
        if train_data:  # Uniknięcie dzielenia przez zero
            avg_final_error_train = temp_error_sum / len(train_data)

    # Zapis modelu
    autoencoder_model_file = "autoencoder_model.pkl"  # Domyślna nazwa dla autoenkodera
    if name == "Iris":
        mlp.save_network(trained_model_file)
    elif name == "Autoencoder":
        suffix = "_bias_on" if use_bias else "_bias_off"
        # Używamy architektury do stworzenia bardziej opisowej nazwy pliku, jeśli jest dostępna
    ...
    # 4. Ewaluacja i wyniki szczegółowe
    if name == "Iris":
        ...
        try:
            ...
            # Inicjalizujemy Trainer z danymi testowymi tylko do ewaluacji, nie do treningu
            ...
            if metrics_test and 'accuracy' in metrics_test:
                if abs(metrics_loaded_iris['accuracy'] - metrics_test['accuracy']) < 1e-6:
                    print(
                        f"  Potwierdzenie: Wydajność wczytanego modelu jest zgodna z oryginalnym ({metrics_test['accuracy']:.4f}).")
                else:
                    print(
                        f"  Uwaga: Wydajność wczytanego modelu ({metrics_loaded_iris['accuracy']:.4f}) różni się nieznacznie od oryginalnego ({metrics_test['accuracy']:.4f}).")
        ...
        if avg_final_error_train is not None:
            ...
            print("Nie udało się obliczyć finalnego błędu MSE dla treningu.")

        print("\n--- Szczegółowe wyniki dla każdego wzorca treningowego Autoenkodera ---")
        # Definicja szerokości kolumn dla czytelności
        # Można je dostosować w zależności od oczekiwanej długości wektorów
        ...
        hidden_layer_index = 0  # Zakładamy, że pierwsza warstwa w mlp.layers to wyjście enkodera

        if not train_data:
            print("Brak danych treningowych do wyświetlenia.")
        else:
            for i, (pattern_in, expected_out) in enumerate(train_data):
                ...
                # Sprawdzamy, czy warstwy istnieją, czy indeks jest poprawny i czy warstwa ma atrybut 'last_output'
                if mlp.layers and \
                        len(mlp.layers) > hidden_layer_index and \
                        hasattr(mlp.layers[hidden_layer_index], 'last_output'):

                    hidden_activations = mlp.layers[hidden_layer_index].last_output
                    if hidden_activations is not None:
                        # Używamy str() dla bardziej kompaktowej reprezentacji niż np.array2string
        ...
        print("\n--- Przykładowe Rekonstrukcje (zbiór testowy/treningowy) ---")
        ...
        if not test_data:
            print("Brak danych testowych do wyświetlenia.")
        else:
        ...
        print("\n--- Test zapisanego modelu Autoenkodera (na zbiorze testowym/treningowym) ---")
        try:
            loaded_mlp_autoencoder = MLP.load_network(autoencoder_model_file)

            total_mse_loaded_model = 0
            if not test_data:
                print("Brak danych testowych do ewaluacji wczytanego modelu.")
            else:
                for x_test, y_test_expected in test_data:
                    y_test_actual = loaded_mlp_autoencoder.forward(x_test)
                    total_mse_loaded_model += loaded_mlp_autoencoder.calculate_pattern_error(y_test_actual,
                                                                                             y_test_expected)

                avg_mse_loaded_model = total_mse_loaded_model / len(test_data) if test_data else 0
                ...
                if avg_final_error_train is not None:
                    if abs(avg_mse_loaded_model - avg_final_error_train) < 1e-6:
                        print(
                            f"  Potwierdzenie: Wydajność wczytanego modelu jest zgodna z oryginalnym ({avg_final_error_train:.8f}).")
                    else:
                        print(
                            f"  Uwaga: Wydajność wczytanego modelu ({avg_mse_loaded_model:.8f}) różni się nieznacznie od oryginalnego ({avg_final_error_train:.8f}).")
